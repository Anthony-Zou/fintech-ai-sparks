...
def main():
    ...
    **Enterprise-Grade Portfolio Management** | Modern Portfolio Theory | Monte Carlo Simulations | Real-Time Risk Analytics
    ...
    with st.sidebar:
        ...
        selected_period = st.selectbox("Historical Data Period:", list(periods.keys()), index=2)
            ...
            with st.spinner("Loading portfolio data..."):
                try:
                    if demo_mode:
                        ...
                        risk_calculator = RiskMetrics(returns_data, risk_free_rate)
                        mc_simulator = MonteCarloSimulator(returns_data, num_simulations=5000, risk_free_rate=risk_free_rate)
                        
                        st.success(f"✅ Demo data loaded for {len(tickers)} assets")
                        
                    else:
                        # Real market data
                        if not YFINANCE_AVAILABLE:
                            st.error("yfinance not available for real market data")
                            return
                        ...
                        optimizer = PortfolioOptimizer(returns_data, risk_free_rate)                        
                        risk_calculator = RiskMetrics(returns_data, risk_free_rate)
                        mc_simulator = MonteCarloSimulator(returns_data, num_simulations=10000, risk_free_rate=risk_free_rate)
                        
                        st.success(f"✅ Market data loaded for {len(tickers)} assets")
...
def risk_dashboard_tab():
    ...
    if 'current_portfolio' not in st.session_state:
        st.warning("⚠️ Please optimize a portfolio first in the Portfolio Builder tab")
        return
    ...
    with col1:
        ...
        with metrics_col1:
            ...
            color_class = "danger-metric" if var_95 < -0.05 else "warning-metric" if var_95 < -0.02 else "success-metric"
        ...
        with metrics_col3:
            ...
            color_class = "success-metric" if max_dd > -0.1 else "warning-metric" if max_dd > -0.2 else "danger-metric"
...
def monitoring_simulation_tab():
            ...
            with st.spinner("Running 10,000 simulations..."):
                try:
                    ...
                    risk_metrics = mc_simulator.calculate_risk_metrics_mc(
                        weights, [0.90, 0.95, 0.99], time_horizon, initial_value
                    )
    ...
    with col2:
        ...
        scenarios_to_run = st.multiselect(
            ...
            ['bull_market', 'bear_market', 'high_volatility', 'recession', 'market_crash', 'recovery'],
        ...
        if st.button("🧪 Run Stress Tests"):
            with st.spinner("Running stress tests..."):
                try:
                    scenario_results = mc_simulator.scenario_analysis(weights, scenarios_to_run)
    ...
    if 'mc_results' in st.session_state:
        ...
        with var_es_col2:
            ...
            if es_data:
                st.dataframe(pd.DataFrame(es_data), hide_index=True, use_container_width=True)
    ...
    if 'scenario_results' in st.session_state:
        ...
        expected_returns = [scenario_results[s]['expected_return'] for s in scenarios]
        volatilities = [scenario_results[s]['volatility'] for s in scenarios]
        var_95_values = [scenario_results[s]['var_95'] for s in scenarios]
        
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Expected Returns', 'Volatility', 'VaR 95%', 'Sharpe Ratios'),
        ...
        # Sharpe ratios
        sharpe_ratios = [scenario_results[s]['sharpe_ratio'] for s in scenarios]
        fig.add_trace(
        ...
        fig.update_layout(height=600, showlegend=False, title="Stress Test Scenario Comparison")
        st.plotly_chart(fig, use_container_width=True)
...
def reports_export_tab():
    ...
    with col1:
        ...
        report_sections = st.multiselect(
            ...
            default=["Executive Summary", "Portfolio Allocation", "Risk Metrics"]
        )
    ...
    summary_data = {
        "Portfolio Metrics": {
            "Total Value": f"${portfolio_data['portfolio_value']:,.0f}",
            "Expected Annual Return": f"{current_portfolio['return']:.2%}",
            "Annual Volatility": f"{current_portfolio['volatility']:.2%}",
            "Sharpe Ratio": f"{current_portfolio['sharpe_ratio']:.3f}",
            "Number of Assets": len(portfolio_data['tickers']),
            "Data Period": portfolio_data['period'],
...
def generate_report(sections, format_type, include_charts):
    ...
    with st.spinner(f"Generating {format_type} report..."):
        try:
            ...
            # 1. Generate PDF using reportlab or similar
            # 2. Create Excel files using openpyxl
...
def create_sample_chart():
    ...
    try:
        ...
        st.info("📊 Interactive charts will be available once dependencies are installed")
        mode='lines',
        name='Portfolio Value',
...
def check_data_availability():
    ...
    if not YFINANCE_AVAILABLE:
        st.sidebar.error("⚠️ yfinance not available - market data fetching disabled")
        return False
    ...
    try:
        ...
        if test_data.empty:
            st.sidebar.warning("⚠️ Unable to fetch market data - check internet connection")
            return False
        return True
    except:
        st.sidebar.warning("⚠️ Unable to fetch market data - check internet connection")
