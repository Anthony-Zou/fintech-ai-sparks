...
class MonteCarloSimulator:
    ...
    def generate_price_paths(self, weights: np.ndarray, time_horizon: int = 252,
        ...
        total_returns = (final_values - initial_portfolio_value) / \
    ...
    def calculate_risk_metrics_mc(self, weights: np.ndarray,
        ...
        simulation_results = self.generate_price_paths(
        ...
        for conf_level in confidence_levels:
            ...
            es_value = np.mean(tail_losses_value) if len(
                tail_losses_value) > 0 else var_value
            es_return = np.mean(tail_losses_return) if len(
                tail_losses_return) > 0 else var_return
    ...
    def stress_test_scenarios(self, weights: np.ndarray,
        ...
        for scenario_name, scenario_params in scenarios.items():
            ...
            shocked_mean = portfolio_mean + \
                scenario_params.get('mean_shock', 0)
            shocked_vol = portfolio_vol * \
                scenario_params.get('vol_multiplier', 1.0)
    ...
    def efficient_frontier_mc(self, num_portfolios: int = 1000,
        ...
        for _ in range(num_portfolios):
            ...
            mean_return = np.mean(mc_results['total_returns'])
            volatility = np.std(mc_results['total_returns'])
    ...
    def portfolio_optimization_mc(self, target_return: float = None,
        ...
        best_portfolio = None
        best_metric = - \
            np.inf if objective in ['sharpe', 'max_return'] else np.inf
        ...
        for _ in range(self.num_simulations):
            ...
            sharpe_ratio = (expected_return - self.risk_free_rate) / \
                volatility if volatility > 0 else 0
            ...
            if objective == 'sharpe':
                ...
                raise ValueError(
                    "Objective must be 'sharpe', 'min_vol', or 'max_return'")
    ...
    def get_simulation_summary(self, weights: np.ndarray,
        ...
        mc_results = self.generate_price_paths(
            weights, time_horizon, initial_value)
